...
#include "third_party/glad/linmath.h"
#include <getopt.h>
...
struct {
  ...
  std::condition_variable p_done; // Condition: particle physics done
  std::condition_variable d_done; // Condition: particle draw done
  std::mutex particles_lock;      // Particles data sharing mutex
} thread_sync;
...
static void draw_particles(GLFWwindow *window, double t, float dt) {
  ...
  std::unique_lock<std::mutex> lock(thread_sync.particles_lock);
  while (!glfwWindowShouldClose(window) &&
    ...
    thread_sync.p_done.wait_until(lock, timeout);
  }
...
static int physics_thread_main(void *arg) {
  ...
  for (;;) {
    ...
    while (!glfwWindowShouldClose(window) &&
      ...
      auto now = std::chrono::system_clock::now();
      auto timeout = now + std::chrono::milliseconds(100);
      thread_sync.d_done.wait_until(lock, timeout);
    }
...
int main(int argc, char **argv) {
  ...
  try {
    std::thread physics_thread(physics_thread_main, window);
