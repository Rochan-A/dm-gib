...
// - Gallery               https://github.com/ocornut/imgui/issues/5243 (please
...
// Getting Started?
// - For first-time users having issues compiling/linking/running or issues
// loading fonts:
//   please post in https://github.com/ocornut/imgui/discussions if you cannot
//   find a solution in resources above.
...
ImGuiSizeCallbackData, ImGuiWindowClass, ImGuiPayload, ImGuiTableSortSpecs,
ImGuiTableColumnSortSpecs)
// [SECTION] Helpers (ImGuiOnceUponAFrame, ImGuiTextFilter, ImGuiTextBuffer,
ImGuiStorage, ImGuiListClipper, ImColor)
...
IMGUI_API void PushAllowKeyboardFocus(
    bool allow_keyboard_focus); // == tab stop enable. Allow focusing using
...
// Style read access
// - Use the style editor (ShowStyleEditor() function) to interactively see what
// the colors are)
...
IMGUI_API ImVec2
GetCursorScreenPos(); // cursor position in absolute coordinates (useful to work
                      // with ImDrawList API). generally top-left ==
                      // GetMainViewport()->Pos == (0,0) in single viewport
                      // mode, and bottom-right == GetMainViewport()->Pos+Size
...
// Tab Bars, Tabs
// Note: Tabs are automatically created by the docking system. Use this to
// create tab bars/tabs yourself without docking being involved.
IMGUI_API bool
...
// _enable_ docking/undocking. About dockspaces:
...
//   - For 'ImGuiKey key' you can still use your legacy native/user indices
//   according to how your backend/engine stored them in io.KeysDown[].
// With IMGUI_DISABLE_OBSOLETE_KEYIO: (this is the way forward)
//   - Any use of 'ImGuiKey' will assert when key < 512 will be passed,
//   previously reserved as native/user keys indices
//   - GetKeyIndex() is pass-through and therefore deprecated (gone if
...
IMGUI_API void RenderPlatformWindowsDefault(
    ...
    void *renderer_render_arg =
        NULL); // call in main loop. will call RenderWindow/SwapBuffers platform
               ...
               // ImGuiViewportFlags_Minimized flag set. May be reimplemented by
...
enum ImGuiInputTextFlags_ {
  ...
  ImGuiInputTextFlags_EscapeClearsAll =
      1 << 20, // Escape key clears content if not empty, and deactivate
               // otherwise (constrast to default behavior of Escape to revert)
...
enum ImGuiTreeNodeFlags_ {
  ...
  ImGuiTreeNodeFlags_NavLeftJumpsBackHere =
      1 << 13, // (WIP) Nav: left direction may move to this TreeNode() from any
...
enum ImGuiTableColumnFlags_ {
  ...
  ImGuiTableColumnFlags_NoHeaderLabel =
      1 << 12, // TableHeadersRow() will not submit label for this column.
...
enum ImGuiHoveredFlags_ {
  ...
  ImGuiHoveredFlags_AllowWhenOverlapped =
      1 << 8, // IsItemHovered() only: Return true even if the position is
              // obstructed or overlapped by another window
  ImGuiHoveredFlags_AllowWhenDisabled =
      1 << 9, // IsItemHovered() only: Return true even if the item is disabled
  ImGuiHoveredFlags_NoNavOverride =
      1 << 10, // Disable using gamepad/keyboard navigation state when active,
               // always query mouse.
  ...
  ImGuiHoveredFlags_NoSharedDelay =
      1 << 13, // Disable shared delay system where moving from one item to the
               // next keeps the previous timer for a short time (standard for
...
enum ImGuiDockNodeFlags_ {
  ...
  ImGuiDockNodeFlags_KeepAliveOnly =
      1
      << 0, // Shared       // Don't display the dockspace node but keep it
            // alive. Windows docked into this dockspace node won't be undocked.
  // ImGuiDockNodeFlags_NoCentralNode              = 1 << 1,   // Shared //
  // Disable Central Node (the node which can stay empty)
  ImGuiDockNodeFlags_NoDockingInCentralNode =
      1 << 2, // Shared       // Disable docking inside the Central Node, which
              // will be always kept empty.
  ImGuiDockNodeFlags_PassthruCentralNode =
      1 << 3, // Shared       // Enable passthru dockspace: 1) DockSpace() will
              // render a ImGuiCol_WindowBg background covering everything
              // excepted the Central Node when empty. Meaning the host window
              // should probably use SetNextWindowBgAlpha(0.0f) prior to Begin()
              // when using this. 2) When Central Node is empty: let inputs
              // pass-through + won't display a DockingEmptyBg background. See
              // demo for details.
  ImGuiDockNodeFlags_NoSplit =
      1 << 4, // Shared/Local // Disable splitting the node into smaller nodes.
              // Useful e.g. when embedding dockspaces into a main root one (the
              // root one may have splitting disabled to reduce confusion).
              // Note: when turned off, existing splits will be preserved.
  ImGuiDockNodeFlags_NoResize = 1
                                << 5, // Shared/Local // Disable resizing node
                                      // using the splitter/separators. Useful
                                      // with programmatically setup dockspaces.
  ImGuiDockNodeFlags_AutoHideTabBar =
      1 << 6, // Shared/Local // Tab bar will automatically hide when there is a
...
enum ImGuiDragDropFlags_ {
  ...
  ImGuiDragDropFlags_SourceNoPreviewTooltip =
      1 << 0, // By default, a successful call to BeginDragDropSource opens a
              ...
              // source contents. This flag disables this behavior.
...
// All our named keys are >= 512. Keys value 0 to 511 are left unused as legacy
// native/opaque key values (< 1.87) Since >= 1.89 we increased typing (went
// from int to enum), some legacy code may need a cast to ImGuiKey.
enum ImGuiKey : int {
  // Keyboard
  ImGuiKey_None = 0,
  ...
  // Gamepad (some of those are analog values, 0.0f to 1.0f) // GAME NAVIGATION
  ...
  ImGuiMod_Shortcut =
      1 << 11, // [Internal, read-only] Alias of ImGuiMod_Super (macOS) or
  // ImGuiMod_Ctrl (otherwise), decided by io.ConfigMacOSXBehaviors.
  ...
  // IsKeyPressed(MY_NATIVE_KEY_CODE)
  ...
  ImGuiKey_KeysData_OFFSET =
      ImGuiKey_NamedKey_BEGIN, // First key stored in io.KeysData[0]. Accesses
                               // to io.KeysData[] must use (key -
                               // ImGuiKey_KeysData_OFFSET).
#else
  ImGuiKey_KeysData_SIZE = ImGuiKey_COUNT, // Size of KeysData[]: hold legacy
                                           // 0..512 keycodes + named keys
  ImGuiKey_KeysData_OFFSET =
      0, // First key stored in io.KeysData[0]. Accesses to io.KeysData[] must
...
enum ImGuiConfigFlags_ {
  ...
  ImGuiConfigFlags_NavEnableKeyboard =
      1 << 0, // Master keyboard navigation enable flag.
...
struct ImGuiIO {
  ...
  float HoverDelayShort;  // = 0.10 sec       // Delay on hovering before
                          // IsItemHovered(ImGuiHoveredFlags_DelayShort) returns
                          // true.
  ...
  bool ConfigViewportsNoTaskBarIcon; // = false          // Disable default OS
                                     ...
                                     // a task bar icon,
                                     // ImGuiViewportFlags_NoTaskBarIcon will be
                                     // set on it.
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  void
      *ImeWindowHandle; // = NULL           // [Obsolete] Set
  ...
  IMGUI_API void AddMouseWheelEvent(float wh_x,
                                    float wh_y); // Queue a mouse wheel update
  ...
  float
      MouseWheelH; // Mouse wheel Horizontal. Most users don't have a mouse with
                   // a horizontal wheel, may not be filled by all backends.
  ImGuiID MouseHoveredViewport; // (Optional) Modify using
...
struct ImGuiTableColumnSortSpecs {
  ...
  ImGuiSortDirection
      SortDirection : 8; // ImGuiSortDirection_Ascending or
                         // ImGuiSortDirection_Descending (you can use this or
                         // SortSign, whichever is more convenient for your sort
                         // function)
...
struct ImGuiListClipper {
  ...
  // Call ForceDisplayRangeByIndices() before first call to Step() if you need a
  // range of items to be displayed regardless of visibility.
  IMGUI_API void ForceDisplayRangeByIndices(
      int item_min,
      int item_max); // item_max is exclusive e.g. use (42, 42+1) to make item
                     // 42 always visible BUT due to alignment/padding of
                     // certain items it is likely that an extra item may be
...
struct ImDrawList {
  ...
  // the end)
  // - FIXME-OBSOLETE: This API shouldn't have been in ImDrawList in the first
  // place!
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  inline void AddBezierCurve(const ImVec2 &p1, const ImVec2 &p2,
                             const ImVec2 &p3, const ImVec2 &p4, ImU32 col,
                             float thickness, int num_segments = 0) {
    AddBezierCubic(p1, p2, p3, p4, col, thickness, num_segments);
  } // OBSOLETED in 1.80 (Jan 2021)
  inline void PathBezierCurveTo(const ImVec2 &p2, const ImVec2 &p3,
...
struct ImDrawData {
  ...
  ImDrawList **CmdLists; // Array of ImDrawList* to render. The ImDrawList are
...
struct ImFontConfig {
  ...
  int OversampleH;  // 3        // Rasterize at higher quality for sub-pixel
                    ...
                    // minimal so you can reduce this to 2 to save memory. Read
...
struct ImFontAtlas {
  ...
  // Consider using ImFontGlyphRangesBuilder to build glyph ranges from textual
  // data.
...
struct ImFont {
  ...
  ImWchar EllipsisChar;  // 2     // out // = '...'    // Character used for
                         // ellipsis rendering.
  ImWchar DotChar; // 2     // out // = '.'      // Character used for ellipsis
                   // rendering (if a single '...' character isn't found)
...
enum ImGuiViewportFlags_ {
  ...
  ImGuiViewportFlags_NoDecoration =
      1 << 3, // Platform Window: Disable platform decorations: title bar,
              ...
              // ImGuiConfigFlags_ViewportsDecoration is set we only set this on
              // popups/tooltips)
  ...
  ImGuiViewportFlags_TopMost =
      1 << 9, // Platform Window: Display on top (for tooltips only).
  ImGuiViewportFlags_Minimized =
      1 << 10, // Platform Window: Window is minimized, can skip render. When
               // minimized we tend to avoid using the viewport pos/size for
               // clipping window or testing if they are contained in the
               // viewport.
  ImGuiViewportFlags_NoAutoMerge =
      1 << 11, // Platform Window: Avoid merging this window into another host
  ...
  ImGuiViewportFlags_CanHostOtherWindows =
      1 << 12, // Main viewport: can host multiple imgui windows (secondary
...
struct ImGuiViewport {
  ...
  ImDrawData *DrawData; // The ImDrawData corresponding to this viewport. Valid
...
struct ImGuiPlatformIO {
  ...
  void (*Platform_CreateWindow)(
      ImGuiViewport *vp); // . . U . .  // Create a new platform window for the
                          // given viewport
  void (*Platform_DestroyWindow)(ImGuiViewport *vp); // N . U . D  //
  void (*Platform_ShowWindow)(
      ImGuiViewport *vp); // . . U . .  // Newly created windows are initially
  ...
  void (*Platform_SetWindowTitle)(
      ImGuiViewport *vp, const char *str); // . . U . .  // Set platform window
  ...
  void (*Platform_OnChangedViewport)(
      ImGuiViewport *vp); // . F . . .  // (Optional) [BETA] FIXME-DPI: DPI
  ...
  int (*Platform_CreateVkSurface)(
      ...
      ImU64 *out_vk_surface); // (Optional) For a Vulkan Renderer to call into
                              // Platform code (since the surface creation needs
                              // to tie them both).

  // (Optional) Renderer functions (e.g. DirectX, OpenGL, Vulkan)
  void (*Renderer_CreateWindow)(
      ImGuiViewport *vp); // . . U . .  // Create swap chain, frame buffers etc.
...
struct ImGuiPlatformMonitor {
  ImVec2 MainPos, MainSize; // Coordinates of the area displayed on this monitor
                            // (Min = upper left, Max = bottom right)
  ImVec2 WorkPos, WorkSize; // Coordinates without task bars / side bars / menu
...
IMGUI_API bool ListBoxHeader(
    ...
    int height_in_items =
        -1); // Helper to calculate size from items_count and height_in_items
...
// AddImageRounded(), PathRect()
typedef ImDrawFlags ImDrawCornerFlags;
enum ImDrawCornerFlags_ {
  ImDrawCornerFlags_None =
      ImDrawFlags_RoundCornersNone, // Was == 0 prior to 1.82, this is now ==
                                    // ImDrawFlags_RoundCornersNone which is !=
                                    // 0 and not implicit
  ImDrawCornerFlags_TopLeft =
      ImDrawFlags_RoundCornersTopLeft, // Was == 0x01 (1 << 0) prior to 1.82.
                                       // Order matches
                                       // ImDrawFlags_NoRoundCorner* flag (we
                                       // exploit this internally).
  ...
  ImDrawCornerFlags_All =
      ImDrawFlags_RoundCornersAll, // Was == 0x0F prior to 1.82
